...
for(e in energy_list){
  ts_hist <- ts(energy05_23_long %>% 
                filter(item==e) %>% 
                pull(value),
                start=c(2005,1), 
                frequency=12)
  fit_arima <- auto.arima(ts_hist) # fit ARIMA
  fc_arima <- forecast(fit_arima, h=12) # forecasting future 12 months
  ARIMA_forecast2024[[e]] <- fc_arima
}

# Store the ARIMA forecast on 2024
